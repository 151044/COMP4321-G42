...
class TableOperationTest {
    ...
    void insertWordInfo() {
        ...
        assertTrue(body.getFrequency(1, 0).contains(new WordInfo(0, 0, 0, 0, "location")));
        assertTrue(title.getFrequency(2, 0).contains(new WordInfo(0, 0, 0, 0, "opportunist")));
        assertFalse(title.getFrequency(2, 0).contains(new WordInfo(0, 0, 0, 0, "opportunion")));
    ...
    @Test
    void getFrequency() {
        assertTrue(body.getFrequency(0, 1).contains(new WordInfo(1, 3270972, 2, 1, "computer")));
        assertFalse(body.getFrequency(0, 1).contains(new WordInfo(1, 3270972, 2, 239040, "computer")));
    }

    @Test
    void deleteFrequencies() {
        body.deleteFrequencies(0);
        assertEquals(0, body.getFrequency(body.getIdFromStem("comput"), 0).size()); // body tables don't have docId == 0
        assertEquals(0, body.getFrequency(body.getIdFromStem("comput"), 0).size()); // title table don't have docId == 0
        assertEquals(2, body.getFrequency(body.getIdFromStem("comput"), 1).size()); // 2 frequency records remaining

        title.deleteFrequencies(0);
        assertEquals(1, title.getFrequency(title.getIdFromStem("comput"), 1).size());
    }
